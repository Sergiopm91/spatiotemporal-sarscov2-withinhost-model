...
% =========================================================================
%  Modelo espaciotemporal de quimiotaxis inmunológica para COVID-19
%  Sistema de 19 ecuaciones diferenciales parciales
%
%  Sistema de unidades:
%    El modelo está adimensionalizado. Las variables representan
%    concentraciones relativas a capacidades de carga:
%      V  = carga viral / V_cap
%      Ma = macrófagos activados / MT
%      Ab = anticuerpos / AT
%
%    Conversión aproximada a unidades físicas (Chimal-Eguia, 2021):
%      V [particles/mL] = V_adim * 1.2e13
%      T cells [cells/mL] = T_adim * 1.5e7
%      B cells [cells/mL] = B_adim * 2.5e8
%      Ab [molecules/mL] = Ab_adim * 3e11
%      Temp [C] = theta
% =========================================================================

%% ===== 1. Dominio espacial y temporal ===================================
Tf = 25;
dt = 0.002;
N  = ceil(Tf / dt);
a_dom = -2; b_dom = 2;
...
fprintf('============================================================\n');
fprintf('  MODELO QUIMIOTAXIS COVID-19 - 19 EDP\n');
fprintf('============================================================\n');
...
% Tasas de muerte o aclaramiento (día^-1)
M1=2.00; M2=2.16; M3=2.00; M4=1.50; M5=1.00;
M6=0.10; M7=0.25; M8=0.10; M9=0.15;
M10=0.06; M11=0.15; M12=0.15;
M13=0.10; M14=0.10;
M15=0.12; M16=0.50; M17=0.02;
...
% Células infectadas, anticuerpos y células B
S_V=0.12;
a52=2.50;
a55=1.50;
a56=0.30;
...
for n = 1:N
    ...
    chi2_func = d2_diss./(d2_diss+Thn).^2;

    Ut(:,:,iMa) = Ma + dt*(D_cell*Lap(:,:,iMa) ...
        + chi_m*chi2_func.*Ma.*Lap(:,:,iThn) ...
        + a30*Mna.*V ...
        + a33*Mna.*V ...
        + act_IFN_Ma ...
        + act_TNF_Ma ...
    ...
    % Condiciones de frontera y positividad
    for k = 1:nv
        tmp = Ut(:,:,k);
        if k==iTheta
...
ylabel('Viral Load');
yyaxis right;
plot(t_axis,Global_Avg(:,iTheta),'b--','LineWidth',1.5);
ylabel('Temp (C)');
title('Virus vs Temperature');
xlabel('Days');
legend('Virus','Temp','Location','northeast');
grid on;
xlim([0 Tf]);
...
hold off;
legend('IL-12','IFN-\gamma','TNF-\alpha','IL-6','IL-10','Location','best');
title('Cytokines');
xlabel('Days');
grid on;
xlim([0 Tf]);
...
hold off;
legend('Tce','Th1','Th2 (x1000)','Location','best');
title('Adaptive Cellular Response');
xlabel('Days');
grid on;
xlim([0 Tf]);
...
hold off;
legend('Antibodies','B-Cells','Location','best');
...
hold off;
legend('Da','Ma','Dna','Mna','Location','best');
title('Innate Cell Dynamics');
xlabel('Days');
grid on;
xlim([0 Tf]);
...
hold off;
legend('Virus','Cv (infectadas)','Location','best');
title('Virus Load and Infected Cells');
xlabel('Days');
grid on;
xlim([0 Tf]);
...
hold on;
xline(4,'--g','Symptoms','LabelVerticalAlignment','top','FontSize',7);
xline(8,'--b','Expected peak','LabelVerticalAlignment','top','FontSize',7);
hold off;
title('Virus vs Chronology');
xlabel('Days');
ylabel('V');
grid on;
xlim([0 25]);
...
hold off;
legend('Tce','Ab','Virus','Location','best');
title('Timing');
xlabel('Days');
grid on;
xlim([0 25]);
...
subplot(2,3,4);
plot(t_axis,Global_Avg(:,iThn),'b-','LineWidth',1.5);
hold on;
plot(t_axis,Global_Avg(:,iTh1),'g-','LineWidth',1.5);
plot(t_axis,Global_Avg(:,iTh2)*1000,'r-','LineWidth',1.5);
hold off;
legend('Thn','Th1','Th2 (x1000)','Location','best');
title('T Helpers');
xlabel('Days');
grid on;
xlim([0 25]);
...
subplot(2,3,5);
plot(t_axis,Global_Avg(:,iTcn),'b-','LineWidth',1.5);
hold on;
plot(t_axis,Global_Avg(:,iTce),'r-','LineWidth',2);
hold off;
legend('Tcn (naive)','Tce (effector)','Location','best');
title('Cytotoxic T');
xlabel('Days');
grid on;
xlim([0 25]);
...
hold off;
legend('TNF-\alpha','IL-6','IL-10','Virus','Location','best');
title('Cytokines vs Virus');
xlabel('Days');
grid on;
xlim([0 25]);

sgtitle('Biological Validation','FontSize',13,'FontWeight','bold');
...
fprintf('\n  --- Métricas clave ---\n');
fprintf('  Carga viral máxima: %.4f  (día %.1f)\n', V_pk, t_axis(V_idx));
fprintf('  Temperatura máxima: %.2f C  (día %.1f)\n', T_pk, t_axis(T_idx));
fprintf('  TNF-a máximo:       %.4f  (día %.1f)\n', tnf_pk, t_axis(tnf_idx));
fprintf('  IL-6 máxima:        %.4f  (día %.1f)\n', il6_pk, t_axis(il6_idx));
fprintf('  Tce máximo:         %.5f  (día %.1f)\n', tce_pk, t_axis(tce_idx));
...
if t_axis(V_idx)>=6 && t_axis(V_idx)<=12
    fprintf('  [OK] Pico viral día %.1f\n', t_axis(V_idx));
else
    fprintf('  [!!] Pico viral día %.1f\n', t_axis(V_idx));
end
if T_pk>=38.5 && T_pk<=40.5
    fprintf('  [OK] Tmax = %.1f C\n', T_pk);
else
    fprintf('  [!!] Tmax = %.1f C\n', T_pk);
end
if t_axis(tce_idx)>=10
    fprintf('  [OK] Tce pico día %.1f\n', t_axis(tce_idx));
else
    fprintf('  [!!] Tce pico día %.1f\n', t_axis(tce_idx));
end
...
fprintf('  Virus < 1%% del pico: %.1f días post-pico\n', vida_media_V);
fprintf('  Tiempo de ejecución: %.1f s\n', elapsed);
fprintf('============================================================\n');
...
function local_make3DGif(xp, yp, Ufinal, idx, gifName, zLab, outDir, frameStride, maxFrames)
    %#ok<INUSD>
    % Genera un GIF de rotación 3D a partir del estado final.
...
% Las siguientes secciones generan figuras, perfiles espaciales,
% validaciones adicionales y análisis numéricos complementarios.

%% ===== A. Panel 19 en inglés ============================================
...
%% ===== B. Resumen de 6 paneles en inglés ================================
...
plot(t_axis,Global_Avg(:,iTh2)*1000,'b-','LineWidth',1.5);
hold off;
legend('T_{ce}','Th_1','Th_2 (\times1000)','Location','best');
title('Adaptive Cellular Response');
xlabel('Days');
grid on;
xlim([0 Tf]);
...
plot(t_axis,Global_Avg(:,iMna),'--','Color',[0 0.6 0.6],'LineWidth',1);
hold off;
legend('D_a','M_a','D_{na}','M_{na}','Location','best');
title('Innate Cell Dynamics');
xlabel('Days');
grid on;
xlim([0 Tf]);
...
plot(t_axis,Global_Avg(:,iCv),'m-','LineWidth',1.5);
hold off;
legend('Virus','C_v (infected)','Location','best');
title('Virus and Infected Cells');
xlabel('Days');
grid on;
xlim([0 Tf]);
...
title('(a) Virus V(x, 0, t)');
legend(time_labels,'Location','best','FontSize',7);
...
title('(b) Infected Cells C_v(x, 0, t)');
legend(time_labels,'Location','best','FontSize',7);
grid on; box on; hold off;
...
title('(e) Temperature \theta(x, 0, t)');
legend(time_labels,'Location','best','FontSize',7);
grid on; box on; hold off;
...
%% ===== D. Trayectoria viral vs ventana clínica ==========================
...
for idt = 1:length(dt_values)
    ...
    for n_t = 1:Nt_test
        ...
        fT=r1*(Vt./(Vt+K_theta)).*log(Vt/r2+1.1).*(Vt>V_star);
...
%% ===== F. Sensibilidad del parámetro de forma RBF =======================
...
results_c = zeros(length(c_values), 3);
...
hold on;
xline(0.5, '--k', 'c = 0.5', 'FontSize', 8, 'LabelVerticalAlignment', 'bottom');
hold off;
xlabel('Shape parameter c');
ylabel('cond(A)');
title('(a) Condition Number vs. c');
grid on;
...
title('(b) Laplacian Approximation Error vs. c');
grid on;
...
%% ===== G. Acoplamiento virus-temperatura en inglés ======================
...
ylabel('V (dimensionless)', 'FontSize', 11);
...
for sv = 1:nvars_snap
    for st = 1:nsnaps
        ...
        title(sprintf('%s at t = %d days', strrep(snap_var_names{sv},'_',' '), tday), ...
            'FontSize',13, 'FontWeight','bold');
...
fprintf('    - panel_19_promedios_EN.png\n');
fprintf('    - resumen_6_paneles_EN.png\n');
fprintf('    - cross_section_profiles.png\n');
fprintf('    - viral_load_clinical_comparison.png\n');
fprintf('    - temporal_convergence.csv\n');
fprintf('    - shape_parameter_sensitivity.csv/png\n');
fprintf('    - virus_temperature_coupling_EN.png\n');
fprintf('============================================================\n');
